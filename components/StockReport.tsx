...
const StockReport: React.FC<StockReportProps> = ({ portfolio, initialTicker = "AAPL" }) => {
  ...
  const isDataStale = (dateString: string) => {
    try {
        const date = new Date(dateString);
        const now = new Date();
        const diffMins = (now.getTime() - date.getTime()) / (1000 * 60);
        return diffMins > 15; // Consider stale if older than 15 minutes
    } catch (e) {
        return false;
    }
  };
  ...
  const dataIsStale = displayData ? isDataStale(displayData.lastUpdated) : false;
        ...
        <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col items-center justify-center text-center rounded-lg p-4 bg-white/5 border border-outline min-h-[180px] relative">
                    ...
                    <div className={`w-full mt-2 pt-2 border-t border-white/10 flex items-center justify-center gap-2 ${dataIsStale ? 'text-orange-400' : 'text-muted'}`}>
                        <span className="material-symbols-outlined text-sm">{dataIsStale ? 'warning' : 'schedule'}</span>
                        <p className="text-xs font-medium">
                            Data as of <span className={`${dataIsStale ? 'text-orange-300' : 'text-gray-300'} font-semibold`}>{displayData.lastUpdated}</span>
                            {dataIsStale && <span className="ml-1 opacity-80">(Delayed)</span>}
                        </p>
                    </div>
                </div>
