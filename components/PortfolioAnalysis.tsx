                ...
                {gridItems.map((stock, index) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`${getGridClasses(index, totalItems)} ${colorClass} flex items-center justify-center rounded-lg p-2 transition-transform hover:scale-[0.98] ${isOthers ? 'cursor-default opacity-80' : 'cursor-pointer'} overflow-hidden border border-white/5`}
                        >
                            <div className="text-center w-full">
                                <span className="text-white font-bold block truncate px-2 text-shadow-sm">{isOthers ? 'Others' : stock.ticker.toUpperCase()}</span>
                                <span className="text-white/90 text-xs sm:text-sm font-medium">{stock.weight}%</span>
                            </div>
                        </div>
                    );
                })}
